...
class ApiService {
  ...
  Future<BookmarkCount> getBookmarkCount() async {
    final token = authProvider.token;
    if (token == null) {
      throw Exception('No token found');
    }
    final url = Uri.parse('$baseUrl/api/bookmarks/count');
    final response = await http.get(
      url,
      headers: {
        'Authorization': 'Bearer $token',
    ...
    if (response.statusCode == 200) {
      ...
      return BookmarkCount.fromJson(body);
    } else {
      throw Exception('Failed to fetch bookmark count: ${response.statusCode}');
    }
  }
